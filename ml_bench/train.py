...
def testing(path, rf_model, nn_model, final_cols, n_pct=0.25, eff_err=0.015):
    src = []
    ...
    for tf in src:
        ...
        train_cats(rf_df)
        categorify(rf_df)
        rf_preds = rf_model.predict(rf_df)
        ...
        nn_preds = nn_model.get_preds(dl=dl)
...
def get_learn(to_nn):
    ...
    learn = tabular_learner(dls, y_range=(0, 1), layers=[500, 250], n_out=1, config={'ps': 0.1}, loss_func=F.mse_loss, metrics=[pt_rmse], path=path)
    return learn
...
if __name__ == '__main__':
    ...
    # NN
    if not ((model_path/'to_nn.pkl').is_file() and (model_path/'nn_m.pth').is_file()):
        print("Training NN model ...")
        if not (model_path/'to_nn.pkl').is_file():
            train_df = pd.read_feather(train_path/'train.feat')
            valid_df = pd.read_feather(train_path/'valid.feat')
            train_df = train_df[train_df['GFlops'] > 0].reset_index(drop=True)
            valid_df = valid_df[valid_df['GFlops'] > 0].reset_index(drop=True)
    ...
    print(f"Validation ...")
    testing(train_path, rf_m, learn, final_cols, n_pct=args.kernel_pct, eff_err=args.eff_err)

    if test_path.is_dir():
        print(f"Testing ...")
        testing(test_path, rf_m, learn, final_cols, n_pct=args.kernel_pct, eff_err=args.eff_err)
